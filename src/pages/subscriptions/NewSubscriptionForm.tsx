...
      
      // التعديل هنا: لا تحفظ البيانات إلا إذا كان هناك رقم هوية مدخل فعلياً
      // لكي لا تظهر رسالة "الكفيل موجود" وهو فارغ عند العودة مرة أخرى
...
        </div>
        <h2 className="text-3xl font-black text-foreground mb-4">تم التسجيل بنجاح!</h2>
        <button 
          onClick={() => window.location.reload()} 
          className="px-10 py-4 rounded-xl gradient-primary text-white font-bold transition-all shadow-card hover:shadow-elevated"
        >
          إضافة مشترك جديد
        </button>
      </div>
...
            previousGuarantor={(guarantorData && (guarantorData.idnumber || guarantorData.IDNumber)) && !guarantorData.isNew ? guarantorData : null}
            previousFormValues={guarantorData?.isNew ? guarantorData : null}
          />
...
         <div className="flex flex-row justify-between items-center gap-4 mt-10 pt-6  border-border">
      <button
        ...
        className="px-10 py-3 rounded-xl bg-secondary text-secondary-foreground font-bold hover:opacity-90 transition-all shadow-sm"
      >
        السابق
      </button>
...
      {/* الخطوة الثالثة: تفاصيل الاشتراك */}
      {currentStep === 3 && (
        <SubscriptionStep
