...
export default function ReturnLoanPage() {
  ...
  const loadLoans = async () => {
    ...
    setLoans((data || []).map((l: any) => ({
      ...l, subscriber_name: l.subscribers?.name || "—",
      subscriber_id_display: l.subscribers?.subscriber_number || "—",
      book_title: l.books?.title || "—", book_barcode: l.books?.barcode || "—",
    })));
  };
  ...
  const handleReturn = async () => {
    ...
    try {
      ...
      if (returnForm.fine > 0) {
        await supabase.from("fines").insert({ loan_id: returnModal.id, subscriber_id: returnModal.subscriber_id, fine_type: returnForm.condition === "good" ? "late_return" : returnForm.condition, amount: returnForm.fine, book_title: returnModal.book_title, notes: returnForm.notes });
      }
      toast({ title: "تم الإرجاع بنجاح ✅" }); setReturnModal(null); setReturnForm({ condition: "good", fine: 0, notes: "" }); loadLoans();
    } catch { toast({ title: "خطأ أثناء الإرجاع", variant: "destructive" }); } finally { setSaving(false); }
  };
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      toast({ title: "تم تجديد الإعارة ✅" }); setRenewModal(null); loadLoans();
    } catch { toast({ title: "خطأ أثناء التجديد", variant: "destructive" }); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="text-3xl font-black text-foreground mb-1">إرجاع كتاب</h1>
        <p className="text-muted-foreground">اختر الكتاب الذي تريد إرجاعه من قائمة الإعارات.</p>
      </div>
      <h4 className="font-bold text-foreground mb-3">قائمة الإعارات الحالية</h4>
      <AgGridTable
        columnDefs={[
      ...
      <Dialog open={!!returnModal} onOpenChange={() => setReturnModal(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>إرجاع كتاب</DialogTitle></DialogHeader>
      ...
      <Dialog open={!!renewModal} onOpenChange={() => setRenewModal(null)}>
        <DialogContent>
          ...
          <p className="text-muted-foreground">هل أنت متأكد من تجديد الإعارة؟ سيتم تمديد المدة 14 يوماً إضافياً.</p>
          <DialogFooter className="gap-2">
            ...
            <button onClick={handleRenew} disabled={saving} className="px-6 py-2 rounded-xl gradient-primary text-white font-bold">{saving ? "..." : "تجديد الإعارة"}</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
